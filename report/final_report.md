...
<center><div style='height:2mm;'></div><div style="font-family:华文楷体;font-size:11pt;">皇甫泊宁，524031910352</div></center>

> 代码地址：https://github.com/smiling689/YOLO-TuneEval，下面报告中提到的文件均在对应位置。
>
...
以下结果来自本人训练输出和 `others/` 目录中三位成员上传的结果。
...
下表展示五张额外真实货架图片在微调前后模型下的检测可视化结果。左列为 COCO 预训练 YOLOv8n 的检测结果，右列为 SKU-110K 微调后 AdamW 模型的检测结果。
...
从可视化结果可以看到，微调前模型在大多数图片上没有形成有效的密集商品检测结果，只有少数 COCO 类别误匹配框；微调后模型能够在货架多层结构、重复包装和密集排列区域输出大量商品框，检测覆盖范围明显扩大。
...
1. 横向比较中同时包含测试集和验证集结果。陈奕莱与本人为测试集结果，卓识与刘易函主要为验证集结果，因此不能作为严格同 split 排名。
2. 陈奕莱测试截图显示有 1 张测试图片被识别为 corrupt image 并被忽略，实际评估为 2935 张图片、431419 个实例，与本人测试集规模 2936 张图片、431546 个实例略有差异。
3. 额外真实图片验证具有定性性质。检测数量能够反映微调前后的明显差异，但不能完全替代人工标注后的定量泛化评估。
